...
type EntryHistoryDto = {
  ...
  productCode: string
  productName: string
  productSize?: string | null        // ✅ NEW: kalau backend kirim langsung
...
const getProductSize = (h: EntryHistoryDto): string | null => {
  return (
    (h as any).productSize ??          // dari field langsung (kalau ada)
    h.snapshot?.product?.size ??       // dari snapshot.product.size
    h.snapshot?.size ??                // atau snapshot.size
    null
  )
}
...
export default function IPQCHistory() {
  ...
  function exportCSV() {
    ...
    const rows = items.map(h => {
      ...
      const size = getProductSize(h)               // ✅ size
      return [
        new Date(h.createdAt).toLocaleString("id-ID"),
        h.action,
        `${h.productCode} — ${h.productName}${size ? ` • ${size}` : ""}`, // ✅ tampilkan size
        h.date.slice(0,10),
        h.shift,
        b, a, bp, ap,
        n.replace(/[\r\n]+/g," "),
      ...
      <section className="hidden md:block rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
        ...
        <div className="w-full overflow-x-auto [-webkit-overflow-scrolling:touch] relative rounded-2xl">
          ...
          <table className="w-full text-sm table-fixed md:min-w-[1200px]">
            {/* ❌ Tanpa <colgroup> untuk menghindari whitespace text node */}
