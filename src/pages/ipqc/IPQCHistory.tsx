...
import API from "@/api/axios" // sesuaikan jika alias '@' belum diset
...
type FinalKeys = "beforeIpqc" | "afterIpqc" | "onGoingPostcured" | "afterPostcured" | "note"
...
// Buat rentang waktu harian lokal → ISO (menghindari mismatch zona waktu di backend)
...
export default function IPQCHistory() {
  // Default dari query URL (klik dari dashboard)
  ...
  // Build querystring (pakai URLSearchParams agar rapi)
  const qs = useMemo(() => {
    const p = new URLSearchParams()
    // penting: filter jenis history IPQC
    p.set("type", "IPQC")
    // tanggal: tetap kirim 'date' (jaga kompatibilitas) + kirim juga rentang harian
    p.set("date", date)
  ...
  // Abort controller (axios -> gunakan signal dari AbortController)
  ...
  async function loadHistory() {
    ...
    try {
      setError(null)
      setLoading(true)
      // NOTE: tanpa prefix /api karena baseURL sudah /api
  ...
  // Export CSV (ikut sertakan size)
  function exportCSV() {
    const header = ["Waktu","Aksi","Produk","Tanggal","Shift","Before","After","Before PC","After PC","Catatan","User","Role"]
    const rows = items.map(h => {
      const b  = Number(pickFinal(h, "beforeIpqc") || 0)
      const a  = Number(pickFinal(h, "afterIpqc") || 0)
      const bp = Number(pickFinal(h, "onGoingPostcured") || 0)
    ...
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:gap-4">
        <div className="flex-1">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">IPQC – Riwayat Penginputan</h1>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:flex items-center gap-1 ml-2">
            ...
            <button
              onClick={()=>{ const d=new Date(); d.setDate(d.getDate()-1); setDate(d.toLocaleDateString("en-CA")); setPage(1) }}
              className="px-2 py-1 rounded-lg text-xs border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800"
            >
              Kemarin
            </button>
            ...
            <div key={h.id} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm">
              ...
              <div className="mt-1">
                ...
                <div className="text-slate-500 text-sm">
                  {h.productName}{size ? ` • ${size}` : ""}
                </div>
                <div className="text-xs text-slate-500 mt-1">{h.date.slice(0,10)} • {h.shift}</div>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-500">Before</span><span className="tabular-nums font-medium">{fmt(b)}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">After</span><span className="tabular-nums font-medium">{fmt(a)}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Before PC</span><span className="tabular-nums font-medium">{fmt(bp)}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">After PC</span><span className="tabular-nums font-medium">{fmt(ap)}</span></div>
              </div>
          ...
          <table className="w-full text-sm table-fixed md:min-w-[1200px]">
            <thead className="sticky top-0 z-20 bg-white/95 dark:bg-slate-900/90 backdrop-blur border-b border-slate-200 dark:border-slate-800">
              <tr className="text-left text-slate-600 dark:text-slate-400">
                ...
                <th className="px-4 py-2 text-right" style={{ width: 120 }}>Before IPQC</th>
                <th className="px-4 py-2 text-right" style={{ width: 120 }}>After IPQC</th>
                <th className="px-4 py-2 text-right" style={{ width: 140 }}>Before Postcured</th>
                <th className="px-4 py-2 text-right" style={{ width: 140 }}>After Postcured</th>
              ...
              {items.map((h) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-right tabular-nums">{fmt(bp)}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{fmt(ap)}</td>
